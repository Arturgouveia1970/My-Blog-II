...
RSpec.describe 'Users pages test', type: :feature do
  describe 'index page' do
    before :each do
      @user_first = User.create(name: 'Grabrielle', photo: 'mybaby.png', bio: 'A cute baby', posts_counter: 0)
      @user_second = User.create(name: 'Joelle', photo: 'mybaby.png', bio: 'A beautiful lady', posts_counter: 1)
      @user_third = User.create(name: 'Artur', photo: 'mybaby.png', bio: 'A good guy', posts_counter: 2)
    end
    it 'check users#index path' do
      visit '/'
      expect(current_path).to eq(root_path)
      expect(page).to have_text('Artur')
    end
    it 'Should display all user names' do
      visit '/'
      expect(page).to have_content('Grabrielle')
      expect(page).to have_content('Joelle')
      expect(page).to have_content('Artur')
    end
  ...
  describe 'show page' do
    before :each do
      @user_first = User.create(name: 'Grabrielle', photo: 'pic.png', bio: 'A cute baby', posts_counter: 2)
    ...
    it 'check users#show path' do
      visit user_path(@user_first.id)
      expect(current_path).to eq(user_path(@user_first.id))
      expect(page).to have_text('A cute baby')
    end
    it 'Should set the link of the picture to the img src' do
      visit user_path(@user_first.id)
      all('img').each do |i|
